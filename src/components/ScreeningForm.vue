...
const investigations = [
  ...
  {name: 'وێنەی تیشکی سنگ و سەردانی پزیشکی پسپۆڕ.', value: CHEST_XRAY, type: LUNG},
  {
    name: 'پشکنینی خوێنی PSA یاخود پشکنینی پرۆستات لەلایەن پزیشکی پسپۆڕەوە هەموو ١ تا ٢ ساڵ جارێک.',
    value: PSA_OR_DRE,
    type: PROSTATE
  },
  {name: 'پشکنینی HPV DNA هەموو ٥ تا ١٠ ساڵ جارێک.', value: HPV, type: CERVICAL},
  {
    name: 'پشکنینی خانەزانی (Cytology) هەموو ٣ ساڵ جارێک تاکو تەمەنی ٤٩ ساڵی، هەموو ٥ ساڵ جارێک لە تەمەنی ٥٠ تاکو ٦٩ ساڵی.',
    value: CITOLOGY,
    type: CERVICAL
  },
  {name: 'پشکنینی پیسایی (FIT) هەموو ساڵ جارێک.', value: FIT, type: COLON},
  {name: 'ئەنجامدانی نازوری کۆڵۆن هەموو ١٠ ساڵ جارێک.', value: COLONOSCOPY_EVRY_TEN_YEAR, type: COLON},
  {
    name: 'ئەنجامدانی نازوری کۆڵۆن لە تەمەنی ٤٠ ساڵی بەرەو سەرەوە هەموو ٥ ساڵ جارێک.',
...
const suggestedInvestigations = ref([SELF_EXAMINATION, CLINICAL_EXAMINATION]);
...
function analyse(e) {
  ...
  // check female cancers => breast, cervical
  if(gender.value === FEMALE) {
    // check breast
    // Age more than 20 => self examination
    console.log(hx.value);
    console.log(fmhx.value);
  }


  // check male cancers => prostate
  if(gender.value === MALE) {
    console.log('male');
  }

  // check both gender cancers => lung, colon
